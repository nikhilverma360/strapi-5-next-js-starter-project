"use client";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { MapPin, Mail, Phone, ArrowRight, Twitter, Facebook, Linkedin, Instagram, Youtube } from 'lucide-react'
interface Link {
  href: string;
  text: string;
  isExternal: boolean;
  isPrimary: boolean;
}

interface FooterProps {
  data: {
    text: string;
    socialLinks: Link[];
  };
}

const YouTubeIcon = () => (
  <svg viewBox="0 0 28.57 20" className="size-5">
    <g>
      <path
        fill="currentColor"
        d="M27.9727 3.12324C27.6435 1.89323 26.6768 0.926623 25.4468 0.597366C23.2197 2.24288e-07 14.285 0 14.285 0C14.285 0 5.35042 2.24288e-07 3.12323 0.597366C1.89323 0.926623 0.926623 1.89323 0.597366 3.12324C2.24288e-07 5.35042 0 10 0 10C0 10 2.24288e-07 14.6496 0.597366 16.8768C0.926623 18.1068 1.89323 19.0734 3.12323 19.4026C5.35042 20 14.285 20 14.285 20C14.285 20 23.2197 20 25.4468 19.4026C26.6768 19.0734 27.6435 18.1068 27.9727 16.8768C28.5701 14.6496 28.5701 10 28.5701 10C28.5701 10 28.5677 5.35042 27.9727 3.12324Z"
      />
      <path
        fill="currentColor"
        d="M11.4253 14.2854L18.8477 10.0004L11.4253 5.71533V14.2854Z"
        className="text-background"
      />
    </g>
  </svg>
);

const TwitterIcon = () => (
  <svg viewBox="0 0 248 204" className="size-5">
    <path
      fill="currentColor"
      d="M221.95 51.29c.15 2.17.15 4.34.15 6.53 0 66.73-50.8 143.69-143.69 143.69v-.04c-27.44.04-54.31-7.82-77.41-22.64 3.99.48 8 .72 12.02.73 22.74.02 44.83-7.61 62.72-21.66-21.61-.41-40.56-14.5-47.18-35.07 7.57 1.46 15.37 1.16 22.8-.87-23.56-4.76-40.51-25.46-40.51-49.5v-.64c7.02 3.91 14.88 6.08 22.92 6.32C11.58 63.31 4.74 33.79 18.14 10.71c25.64 31.55 63.47 50.73 104.08 52.76-4.07-17.54 1.49-35.92 14.61-48.25 20.34-19.12 52.33-18.14 71.45 2.19 11.31-2.23 22.15-6.38 32.07-12.26-3.77 11.69-11.66 21.62-22.2 27.93 10.01-1.18 19.79-3.86 29-7.95-6.78 10.16-15.32 19.01-25.2 26.16z"
    />
  </svg>
);

const GitHubIcon = () => (
  <svg viewBox="0 0 438.549 438.549" className="size-5">
    <path
      fill="currentColor"
      d="M409.132 114.573c-19.608-33.596-46.205-60.194-79.798-79.8-33.598-19.607-70.277-29.408-110.063-29.408-39.781 0-76.472 9.804-110.063 29.408-33.596 19.605-60.192 46.204-79.8 79.8C9.803 148.168 0 184.854 0 224.63c0 47.78 13.94 90.745 41.827 128.906 27.884 38.164 63.906 64.572 108.063 79.227 5.14.954 8.945.283 11.419-1.996 2.475-2.282 3.711-5.14 3.711-8.562 0-.571-.049-5.708-.144-15.417a2549.81 2549.81 0 01-.144-25.406l-6.567 1.136c-4.187.767-9.469 1.092-15.846 1-6.374-.089-12.991-.757-19.842-1.999-6.854-1.231-13.229-4.086-19.13-8.559-5.898-4.473-10.085-10.328-12.56-17.556l-2.855-6.57c-1.903-4.374-4.899-9.233-8.992-14.559-4.093-5.331-8.232-8.945-12.419-10.848l-1.999-1.431c-1.332-.951-2.568-2.098-3.711-3.429-1.142-1.331-1.997-2.663-2.568-3.997-.572-1.335-.098-2.43 1.427-3.289 1.525-.859 4.281-1.276 8.28-1.276l5.708.853c3.807.763 8.516 3.042 14.133 6.851 5.614 3.806 10.229 8.754 13.846 14.842 4.38 7.806 9.657 13.754 15.846 17.847 6.184 4.093 12.419 6.136 18.699 6.136 6.28 0 11.704-.476 16.274-1.423 4.565-.952 8.848-2.383 12.847-4.285 1.713-12.758 6.377-22.559 13.988-29.41-10.848-1.14-20.601-2.857-29.264-5.14-8.658-2.286-17.605-5.996-26.835-11.14-9.235-5.137-16.896-11.516-22.985-19.126-6.09-7.614-11.088-17.61-14.987-29.979-3.901-12.374-5.852-26.648-5.852-42.826 0-23.035 7.52-42.637 22.557-58.817-7.044-17.318-6.379-36.732 1.997-58.24 5.52-1.715 13.706-.428 24.554 3.853 10.85 4.283 18.794 7.952 23.84 10.994 5.046 3.041 9.089 5.618 12.135 7.708 17.705-4.947 35.976-7.421 54.818-7.421s37.117 2.474 54.823 7.421l10.849-6.849c7.419-4.57 16.18-8.758 26.262-12.565 10.088-3.805 17.802-4.853 23.134-3.138 8.562 21.509 9.325 40.922 2.279 58.24 15.036 16.18 22.559 35.787 22.559 58.817 0 16.178-1.958 30.497-5.853 42.966-3.9 12.471-8.941 22.457-15.125 29.979-6.191 7.521-13.901 13.85-23.131 18.986-9.232 5.14-18.182 8.85-26.84 11.136-8.662 2.286-18.415 4.004-29.263 5.146 9.894 8.562 14.842 22.077 14.842 40.539v60.237c0 3.422 1.19 6.279 3.572 8.562 2.379 2.279 6.136 2.95 11.276 1.995 44.163-14.653 80.185-41.062 108.068-79.226 27.88-38.161 41.825-81.126 41.825-128.906-.01-39.771-9.818-76.454-29.414-110.049z"
    />
  </svg>
);

function renderIcon(text: string) {
  switch (text) {
    case "twitter":
      return <TwitterIcon />;
    case "github":
      return <GitHubIcon />;
    case "youtube":
      return <YouTubeIcon />;
    default:
      return null;
  }
}

function Footer1({text, socialLinks}: {text: string, socialLinks: Link[]}){
  return (
    <footer className="container flex flex-col items-center justify-between gap-6 py-10 sm:flex-row">
    <p className="text-center text-sm">&copy; {new Date().getFullYear()} {text}</p>
    <div className="flex items-center gap-5">
      {socialLinks &&
        socialLinks.map((link) => (
          <Link
            href={link.href}
            className="text-muted-foreground hover:text-foreground"
            key={link.text}
            target="_blank"
          >
            {renderIcon(link.text?.toLowerCase() ?? "")}
          </Link>
        ))}
    </div>
  </footer>
  );
}

const Footer2= ({text, socialLinks}: {text: string, socialLinks: Link[]}) => (
  <footer className="bg-[#fafafa] px-4 sm:px-6 md:px-8 lg:px-24 xl:px-24 2xl:px-48 text-gray-800">
  <div className="container-custom">
    {/* Main Footer Content */}
    <div className="py-12 sm:py-16 border-b border-gray-300">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-6">
        {/* Company Information */}
        <div className="lg:col-span-1 sm:col-span-2">
          <div className="flex items-center space-x-3 mb-4 sm:mb-6">
            {/* Logo Image */}
            <img 
              src="/banger_finlab_logo-05-removebg-preview 2.png" 
              alt="Banger Finlab Logo" 
              className="w-32 h-32 object-contain"
            />
          </div>
          
          <p className="text-gray-700 mb-4 sm:mb-6 leading-relaxed text-sm">
            Precision finance, custom strategies, and real results. Banger Finlab provides expert financial advisory services backed by research and case studies.
          </p>
          
          <div className="space-y-2 sm:space-y-3">
            <div className="flex items-start space-x-3 text-gray-700">
              <MapPin className="w-4 h-4 text-[#397BA7] mt-0.5 flex-shrink-0" />
              <span className="text-sm">1234 Finance Plaza, Suite 500<br />New York, NY 10001</span>
            </div>
            <div className="flex items-center space-x-3 text-gray-700">
              <Mail className="w-4 h-4 text-[#397BA7] flex-shrink-0" />
              <span className="text-sm">contact@bangerfinlab.com</span>
            </div>
            <div className="flex items-center space-x-3 text-gray-700">
              <Phone className="w-4 h-4 text-[#397BA7] flex-shrink-0" />
              <span className="text-sm">+1 (555) 123-4567</span>
            </div>
          </div>
        </div>

        {/* About Links */}
        <div className="sm:col-span-1 lg:mt-20 lg:col-span-1">
          <h4 className="font-bold text-base sm:text-lg mb-3 sm:mb-4 text-[#397BA7]">About</h4>
          <ul className="space-y-2">
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">About Banger Finlab</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Core Values</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Founders</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Careers</a></li>
          </ul>
        </div>

        {/* Services Links */}
        <div className="sm:col-span-1  lg:mt-20 lg:col-span-1">
          <h4 className="font-bold text-base sm:text-lg mb-3 sm:mb-4 text-[#397BA7]">Services</h4>
          <ul className="space-y-2">
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Financial Advisory</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Strategy Drafting</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Case Study Research</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Your Banger Strategy</a></li>
          </ul>
        </div>

        {/* Resources Links */}
        <div className="sm:col-span-1  lg:mt-20 lg:col-span-1">
          <h4 className="font-bold text-base sm:text-lg mb-3 sm:mb-4 text-[#397BA7]">Resources</h4>
          <ul className="space-y-2">
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Banger Capsules</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Case Studies</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">Blog</a></li>
            <li><a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 text-sm">FAQ</a></li>
          </ul>
        </div>

        {/* Newsletter Subscription */}
        <div className="sm:col-span-2  lg:mt-20 lg:col-span-1">
          <h4 className="font-bold text-base sm:text-lg mb-3 sm:mb-4 text-[#397BA7]">Newsletter</h4>
          <p className="text-gray-700 mb-4 text-sm">
            Subscribe to our newsletter for the latest financial insights and strategies.
          </p>
          
          <div className="flex flex-col sm:flex-row lg:flex-col space-y-2 sm:space-y-0 sm:space-x-2 lg:space-x-0 lg:space-y-2">
            <input
              type="email"
              placeholder="Enter your email"
              className="flex-1 bg-[#1E2949] text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-amber-500 transition-colors duration-200 text-sm placeholder-white/70"
            />
            <button className="bg-[#E2B33C] text-white px-4 py-2 sm:py-3 rounded-lg hover:bg-amber-600 transition-colors duration-200 flex items-center justify-center space-x-2 text-sm font-medium">
              <span>Subscribe</span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>
    </div>

    {/* Bottom Footer */}
    <div className="py-4 sm:py-6 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
      {/* Left: Copyright */}
      <div className="text-gray-700 text-sm text-center md:text-left">
        © 2025 Banger Finlab. All rights reserved.
      </div>
      
      {/* Center: Terms, Privacy, Cookie Policy */}
      <div className="flex flex-wrap justify-center space-x-4 text-sm">
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200">Terms of Service</a>
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200">Privacy Policy</a>
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200">Cookie Policy</a>
      </div>

      {/* Right: Social Media Icons */}
      <div className="flex space-x-3 sm:space-x-4">
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 p-1">
          <Twitter className="w-4 h-4 sm:w-5 sm:h-5" />
        </a>
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 p-1">
          <Facebook className="w-4 h-4 sm:w-5 sm:h-5" />
        </a>
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 p-1">
          <Linkedin className="w-4 h-4 sm:w-5 sm:h-5" />
        </a>
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 p-1">
          <Instagram className="w-4 h-4 sm:w-5 sm:h-5" />
        </a>
        <a href="#" className="text-gray-700 hover:text-amber-500 transition-colors duration-200 p-1">
          <Youtube className="w-4 h-4 sm:w-5 sm:h-5" />
        </a>
      </div>
    </div>
  </div>
</footer>
);

export function Footer({ data }: Readonly<FooterProps>) {
  if (!data) return null;
  const { text, socialLinks } = data;
  const pathname = usePathname();
  return (
    <>
      {pathname === "/" ? <Footer1 text={text} socialLinks={socialLinks} /> : <Footer2 text={text} socialLinks={socialLinks} />}
    </>
  );
}
